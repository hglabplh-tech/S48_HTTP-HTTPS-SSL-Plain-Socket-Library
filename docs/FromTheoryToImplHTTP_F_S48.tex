...
\hline
\hline
\abbrhighcol{UDP} & \abbrhighcol{QUIC} \\
\hline
\specialcell{W / O Connection and stateless \\ peer waits for packages} & \specialcell{Connection  oriented stateful \\ Client / Server connection}\\ 
 \hline
 \specialcell{The package order is not defined. \\ Applications have to look for \\ the order themselves}   & \specialcell{The package order is well defined \\ but there is also the possibility\\. to send packages unordered}   \\
 \hline  
 \specialcell{Package loss is not handled}  & \specialcell{Has a Package loss detection \\ and handles a Package loss} \\
 \hline  
\specialcell{nothing for reset a transmission} & \specialcell{a transmission can be resetted} \\
 \hline 
 \end{tabular}
 \newline
 \newline
 Table 2: \abbrhighcol{Differences between UDP and QUIC which is built on it}
\label{tab:DivUDP_QUIC}
\end{flushleft}
\begin{multicols}{2}
\section{References}
